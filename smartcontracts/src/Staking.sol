...
contract Staking is IStaking, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _token, uint256 _minimumStake, uint32 _lockPeriod)
        external
        payable
        virtual
        initializer
    ...
    function __Staking_init_unchained(address _token, uint256 _minimumStake, uint32 _lockPeriod)
        internal
        onlyInitializing
    {
        token = _token;
     ...
     */
    function hasAvailableStake(address _staker) external view override returns (bool) {
     ...
     */
    function getAllocation(address _escrowAddress) external view override returns (Allocation memory) {
     ...
     * @return Allocation data
     */
    function _getAllocation(address _escrowAddress) private view returns (Allocation memory) {
     ...
     */
    function getAllocationState(address _escrowAddress) external view override returns (AllocationState) {
    ...
    function _getAllocationState(address _escrowAddress) private view returns (AllocationState) {
        ...
        if (
            allocation.closedAt == 0
                && (escrowStatus == IEscrow.EscrowStatuses.Complete || escrowStatus == IEscrow.EscrowStatuses.Cancelled)
        ) {
     ...
     */
    function getStaker(address _staker) external view override returns (Stakes.Staker memory) {
     ...
     */
    function getListOfStakers() external view override returns (address[] memory, Stakes.Staker[] memory) {
    ...
    function unstake(uint256 _tokens) external override {
        ...
        require(_tokens > 0, "Must be a positive number");
        require(staker.tokensAvailable() >= _tokens, "Insufficient amount to unstake");
     ...
     */
    function slash(address _slasher, address _staker, address _escrowAddress, uint256 _tokens)
        external
        override
        onlyOwner
    {
        ...
        IRewardPool(rewardPool).addReward(_escrowAddress, _staker, _slasher, _tokens);
     ...
     */
    function allocate(address _escrowAddress, uint256 _tokens) external override {
     ...
     * @param _tokens Amount of tokens to allocate
     */
    function _allocate(address _staker, address _escrowAddress, uint256 _tokens) private {
        require(_escrowAddress != address(0), "Must be a valid address");
        require(stakes[msg.sender].tokensAvailable() >= _tokens, "Insufficient amount of tokens in the stake");
        require(_tokens > 0, "Must be a positive number");
        require(_getAllocationState(_escrowAddress) == AllocationState.Null, "Allocation already exists");
        ...
        emit StakeAllocated(_staker, allocation.tokens, _escrowAddress, allocation.createdAt);
    }
    ...
    function _closeAllocation(address _escrowAddress) private {
        Allocation storage allocation = allocations[_escrowAddress];
        require(allocation.staker == msg.sender, "Only the allocator can close the allocation");
        ...
        emit AllocationClosed(allocation.staker, _tokens, _escrowAddress, allocation.closedAt);
    }
